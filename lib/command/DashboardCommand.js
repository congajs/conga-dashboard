...
module.exports = class DashboardCommand extends AbstractCommand {
    ...
    execute(input, output, next) {
        ...
        const frameworkApp = this.container.get('config').get('framework').app;
        ...
        this.container.get('conga.webpack.builder').build(config, () => {
            ...
            app.get('/_conga/ping', (req, res) => {
                tcpp.probe('localhost', appConfig.port, (err, available) => {
                    res.json({
                        available: available
                    });
                });
            ...
            app.all('/_conga/*', (req, res) => {
                ...
                try {
                    ...
                    const x = request('http://localhost:' + appConfig.port + req.path);
                    req.pipe(x).on('error', () => {
