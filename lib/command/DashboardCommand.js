    ...
    execute(input, output, next) {
        ...
        this.container.get('conga.webpack.builder').build(config, () => {
            ...
            app.get('/_conga/ping', (req, res) => {
                tcpp.probe('localhost', this.container.get('config').get('framework').app.port, (err, available) => {
                    res.json({
                        available: available
                    });
                });
            ...
            app.all('/_conga/*', (req, res) => {
                ...
                try {
                    ...
                    const x = request('http://localhost:' + this.container.get('config').get('framework').app.port + req.path);
                    req.pipe(x).on('error', () => {
            ...
            server.listen(this.container.get('config').get('dashboard').port);
            output.writeln('dashboard running on port ' + this.container.get('config').get('dashboard').port);
